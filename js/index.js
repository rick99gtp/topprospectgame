...
console.log(random_quotes.length);
console.log(random_quotes_author.length);
...
function startQuoteGenerator() {
    ...
    setInterval(function() {
        let newQuoteNumber = Math.floor(Math.random() * random_quotes.length);
        random_quote.innerHTML = random_quotes[newQuoteNumber];
        random_quote_author.innerHTML = random_quotes_author[newQuoteNumber];
    }, 6000);
}
...
window.addEventListener('scroll', function() {
	...
	if(position_1.top < (window.innerHeight - 100) && position_1.bottom >= 0) {
        element_1.style.animation = "el-fade-in 6s ease-in-out infinite";
        element_2.style.animation = "el-fade-in 6s ease-in-out infinite";
